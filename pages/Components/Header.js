function Header(props) {
    ...
    const selectHandler = e => {
        props.setBase(props.props.data.filter(el => el.genres.some(elem => elem === e)));
    }
    ...
    return (
        ...
        <input type="text" onChange={() => searchHandler(event.target.value)}></input>
        <select onChange={() => selectHandler(event.target.value)}>
            {getGenres().map((el, index, arr) => <option key={`genre_${index}`} value={el}>{el}</option>)}
        </select>
        </div>
    )
}
